...
public class UserControllerFullStack {
    ...
    public String showNewForm(Model model) {
        model.addAttribute("AddNewUser", new Users()); //Đặt tên AddNewUser, gọi tên vào th:object="${AddNewUser}" trong user_form_Update.html
        ...
        return "user_form_Add"; //Trả về trang user_form_Add.html để điền các thông tin của 1 user, form thêm đã ràng buộc id trong Users2
    }

    //Create: dùng POST method, editSave
    @PostMapping("/ListUsers/save") //Đặt tên đường dẫn, viết lại đường dẫn trang users_form.html sẽ thấy đường dẫn th:action="@{/ListUsers/save}" method="post" th:object="${EditUser}"
    public String saveUser (Users EditUser, RedirectAttributes ra) {
            ...
            return "redirect:/api/v1/ListUsers"; //Trả về đường dẫn @GetMapping ("/ListUsers"), trang lstUsers.html
    }

    //Create: dùng POST method, save
    @PostMapping("/ListUsers/save2") //Đặt tên đường dẫn, viết lại đường dẫn trang users_form2.html sẽ thấy đường dẫn th:action="@{/ListUsers/save}" method="post" th:object="${AddNewUser}"
    public String saveUser2 (Users2 AddNewUser, RedirectAttributes ra) {
        service.save2(AddNewUser); //Lấy thông tin user theo mã id, khi người dùng nhấn vào edit hoặc delete trong danh sách user_form_Update.html
    ...
    //Read: dùng GET method
    // Cách 2. Dựa vào hàm editSave. Khi nhấn nút Delete sẽ đưa toàn bộ thông tin dữ liệu id đó lên user_form3. Từ user_form_Delete.html Khi nhấn nút Delete sẽ cập nhật lại trường enable true thành false
    @GetMapping("/ListUsers/delete/{id}") //Đặt tên đường dẫn, viết lại đường dẫn trang lstusers.html, đoạn code edit và delete theo mã id, th:href="@{'/ListUsers/edit/' + ${user.id}}"
    public String showDeleteForm(@PathVariable("id") Integer id, Model model, RedirectAttributes ra) {
        try {
            ...
            return "user_form_Delete"; //Trả về trang user_form_Update.html, dùng để điền các thông tin của 1 user, editSave
        } catch (UserNotFoundException e) {
            ra.addFlashAttribute("message", e.getMessage()); //e.getMessage sẽ lấy thông báo từ UserService của hàm save. Thông báo lỗi, thông báo sẽ hiển thị ở trang danh sách users, lstUsers.html
            return "redirect:/api/v1/ListUsers"; //Trả về đường dẫn @GetMapping ("/ListUsers"), trang lstUsers.html
        }
